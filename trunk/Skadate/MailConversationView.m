...
@synthesize  table,messageViewLabel,nameArray,messageId,senderId,messageContentArray,timeArray,urlReq,respData,indicatorView,indicatorLabel,objIndicatorView,messageID,senderID,btnReply,btnDelete,btnMailBox,btnBookMark,subject,conversNum,conversationNom,previousBut,lblOfTxt,thumbPicURLs,imageArr,domain;
...
- (void)viewDidLoad
{
    ...
    [conversationNom setTextAlignment:NSTextAlignmentCenter];
...
-(IBAction) clickedSegmentContol
{
    ...
    {       
        ...
        messageId=[rootConIDArray objectAtIndex:index];
        NSURL *url = [NSURL URLWithString:[urlReq stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding]];
        NSURLRequest *urlrequest = [NSURLRequest requestWithURL:url];
        [[[NSURLConnection alloc] initWithRequest:urlrequest delegate:self] autorelease];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath 
{
   ...
   NSString *localFilePath = [originalPath stringByAppendingPathComponent:imageName];
    ...
    lblOfTxt.textAlignment = NSTextAlignmentLeft;
