...
@synthesize objIndicatorView,dividerLineImage;
@synthesize flagHome;
...
-(void)deletePhotos
{
    ...
    NSString *req = [NSString stringWithFormat:@"%@/mobile/Delete_photos/?pid=%@&skey=%@&Count=%d&photo_id=%@",domain,((SkadateAppDelegate *) [UIApplication sharedApplication].delegate).loggedProfileID,((SkadateAppDelegate *)[UIApplication sharedApplication].delegate).loggedSessionID, (int)[selectedThumbPicURLs count],selectedPhotoIdsToSend];
    ...
    deletePhotoCon=[[[NSURLConnection alloc] initWithRequest:urlrequest delegate:self] autorelease];
}
...
-(void)reloadPhotos
{
    ...
    loadPhotoCon=[[[NSURLConnection alloc] initWithRequest:urlrequest delegate:self] autorelease];
    btnEdit.hidden=YES;
}
...
- (void)connectionDidFinishLoading:(NSURLConnection *)connection
{
    ...
    else
    {
        ...
        else if(connection==deletePhotoCon)
        {
            NSString *messegeStr=(NSString*)[parsedData objectForKey:@"Message"];
...
-(IBAction)buttonAction:(id)sender
{
    if([myphotolab.text isEqualToString:@"My Photos"])
    {
        ...
        objPhotoFullView.resultCount = (int)[thumbPicURLs count];
    ...
    else 
    {
        ...
        if([selectedThumbPicURLs count]>0)
        {
            ...
            {
                [myphotolab setText:[NSString stringWithFormat:@"%d Item Selected", (int)[selectedThumbPicURLs count]]];
            }
            else
            {
                [myphotolab setText:[NSString stringWithFormat:@"%d Items Selected", (int)[selectedThumbPicURLs count]]];
            }
        }
        else
        {
