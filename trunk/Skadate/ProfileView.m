...
@synthesize profileID,indicatorView,indicatorLabel,objIndicatorView,profilenamestring;
...
- (void)viewDidLoad
{
    ...
    [profilelab setTextAlignment:NSTextAlignmentCenter];
    ...
    Profile_ViewUrlConnection=[[[NSURLConnection alloc] initWithRequest:urlrequest delegate:self] autorelease];
    ...
    UIImage* seg0 = [UIImage imageNamed:@"Info_tab_over.png"];
    ...
    UIImage* seg1 = [UIImage imageNamed:@"Photos_tab.png"];
    if ([UIImage instancesRespondToSelector:@selector(imageWithRenderingMode:)])
...
{    
    ...
    objComposeMessageView.selectedprofileid = (int)[profileID integerValue];
...
- (void)actionSheet:(UIActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex
{
    ...
    /*if (selectImg==6)
    {
        if (buttonIndex==0)
        {
            ...
            NSString *req = [NSString stringWithFormat:@"%@/mobile/Send_Kiss/?sid=%@&rid=%@&skey=%@",domain,(((SkadateAppDelegate *) [UIApplication sharedApplication].delegate).loggedProfileID),profileID,((SkadateAppDelegate *) [UIApplication sharedApplication].delegate).loggedSessionID]; 
            NSURL *url = [NSURL URLWithString:[req stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding]];
            NSURLRequest *urlrequest = [NSURLRequest requestWithURL:url];
            kissUrlConnection=[[[NSURLConnection alloc] initWithRequest:urlrequest delegate:self] autorelease];
            self.view.userInteractionEnabled=NO;
        }
    }
    else
...
{    
    ...
    if (connection==Profile_ViewUrlConnection)
    {
