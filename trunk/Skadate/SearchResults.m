...
@synthesize urlReq,indicatorView,indicatorLabel,objIndicatorView,objChatMembersView,objSearchMembersView,objComposeMessageView;
...
- (void)viewDidLoad
{
    ...
    [searchresultslable setTextAlignment:NSTextAlignmentCenter];
    ...
    NSURL *url = [NSURL URLWithString:[urlReq1 stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding]];
    ...
    if (actionSheet.tag == 4)
    {
        if (buttonIndex == 0) 
        {
            ...
            else
            {
                ...
                NSURL *url = [NSURL URLWithString:[urlReqSave stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding]];
                NSURLRequest *urlrequest = [NSURLRequest requestWithURL:url];
                [[[NSURLConnection alloc] initWithRequest:urlrequest delegate:self] autorelease];
                [self ShowIndicatorView:@"Saving..."];
                self.view.userInteractionEnabled=NO;
            }
        }
    }
...
-(IBAction)clickedSavedButton: (id) sender
{
    ...
    txtFldInAlert = [myAlert textFieldAtIndex:0];// [[UITextField alloc] initWithFrame:CGRectMake(15, 50, 270, 20)];
...
{    
    ...
    parsedData = (NSDictionary *)[parser objectWithString:responseString error:&error];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath 
{
    if (indexPath.section==0) 
    {
        if (indexPath.row==nameArray.count) 
        {
            isLoading=YES;
            NSString *urlReq1;
            NSString *urlStartAndLimits=[NSString stringWithFormat: @"&start=%d&limit=10", (int)[nameArray count]];
