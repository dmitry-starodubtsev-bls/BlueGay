...
- (void)dealloc
{
    [self.appDelegateTimer invalidate];
    self.appDelegateTimer = nil;
}
...
{    
    ...
    {
        profilePic=[NSString stringWithFormat:@"%@%@",self.domain,profilePic];
        
        NSData *mydata = [[NSData alloc] initWithContentsOfURL:[NSURL URLWithString:profilePic]] ;
        if (mydata) 
        {
...
- (void )fetchNotificationData
{
    ...
    if (loggedId)
    {
        ...
        {            
            ...
            self.domain = [prefs stringForKey:@"URL"];
            
            NSString *urlReq = [NSString stringWithFormat:@"%@/mobile/NotificationCount/?pid=%@&skey=%@",self.domain,((SkadateAppDelegate *) [UIApplication sharedApplication].delegate).loggedProfileID,((SkadateAppDelegate *)[UIApplication sharedApplication].delegate).loggedSessionID];
            ...
            dispatch_async(queue, ^{
                ...
                [wSelf urlResponseData:receivedData];
...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions
{
    ...
    self.redNavbar=230.0;
    self.greenNavbar=230.0;
    self.blueNavbar=230.0;
    
    self.redNavBorder=173.0;
    self.greenNavBorder=173.0;
    self.blueNavBorder=173.0;
    
    self.username = [[NSString alloc]init];
    self.password = [[NSString alloc]init];
    self.siteURL = [[NSString alloc]init];
    
    self.fontNavTitle=[UIFont fontWithName:@"Helvetica-Bold" size:22];
    
    self.fontHomeTitle=[UIFont fontWithName:@"Helvetica-Bold" size:12];
    ...
    self.splashView = [UIImageView alloc];
    
    if (UI_USER_INTERFACE_IDIOM()==UIUserInterfaceIdiomPhone)
    {
        self.splashView = [self.splashView initWithFrame:CGRectMake(0,0, 320, 480)];
    ...
    self.appDelegateinvocation= [NSInvocation invocationWithMethodSignature:
                 [self methodSignatureForSelector: @selector(fetchNotificationData)]];
    [self.appDelegateinvocation setTarget:self];
    [self.appDelegateinvocation setSelector:@selector(fetchNotificationData)];
    
    if (self.appDelegateTimer)
    {        
        [self.appDelegateTimer invalidate];
        self.appDelegateTimer = nil;
    }
    
    self.appDelegateTimer = [NSTimer scheduledTimerWithTimeInterval:3.0 invocation:self.appDelegateinvocation repeats:YES];
...
{    
    ...
    {        
        ...
        UIViewController* detailViewController = [[detailClass alloc] initWithNibName:nil bundle:nil] ;
